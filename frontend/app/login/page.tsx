...
const AUTH_ERROR_MESSAGES: Record<string, string> = {
  google_not_configured: "Google OAuth 클라이언트 ID가 아직 설정되지 않았어요. 데모 로그인으로 먼저 흐름을 확인할 수 있습니다.",
  invalid_oauth_state: "로그인 세션이 만료되었어요. 다시 시도하거나 데모 로그인을 사용해 주세요.",
  missing_oauth_code: "Google 로그인 응답이 올바르지 않았어요. 다시 시도하거나 데모 로그인을 사용해 주세요.",
  access_denied: "Google 로그인이 취소되었어요. 다시 시도하거나 데모 로그인을 사용해 주세요.",
};
...
function getAuthErrorMessage(error: string | undefined, description: string | undefined) {
  ...
  if (error === "org_internal") {
    return "현재 Google OAuth 앱이 조직 내부 사용자 전용으로 설정되어 있어요. 개인 Gmail 계정으로는 접근할 수 없으니, Google Cloud Console에서 앱 대상을 External로 바꾸고 테스트 사용자를 추가하거나 데모 로그인을 사용해 주세요.";
  }
  return AUTH_ERROR_MESSAGES[error] ?? description ?? "Google 로그인 중 문제가 발생했어요. 데모 로그인으로 먼저 추천 흐름을 확인할 수 있습니다.";
}
  ...
  return (
    <section className="flex min-h-[calc(100vh-88px)] items-start justify-center bg-gift-cream px-5 pb-20 pt-36">
      <div className="soft-shell w-full max-w-[620px] rounded-[36px] px-12 py-12 text-center">
        ...
        <h1 className="display-title mt-10 text-[34px] font-black text-gift-ink">시작하기 전에 잠깐만요</h1>
        ...
        {authErrorMessage ? (
          ...
          <div className="mx-auto mt-6 rounded-[24px] border border-gift-line bg-gift-soft px-6 py-4 text-left text-[15px] leading-7 text-gift-muted">
            <p className="font-black text-gift-ink">개발 중 빠른 확인</p>
            <p className="mt-2">
              Google OAuth가 조직/테스트 사용자 설정에 막히면 데모 로그인으로 추천 생성과 기록 저장 흐름을 먼저 확인할 수 있어요.
            </p>
          </div>
        )}
