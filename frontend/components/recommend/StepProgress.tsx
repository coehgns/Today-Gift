...
export function StepProgress({ steps, currentStep }: { steps: string[]; currentStep: number }) {
  return (
    <div className="rounded-[1.5rem] border border-gift-line bg-white/60 p-3 shadow-sm">
          ...
          <div key={step} className="flex min-w-0 flex-1 flex-col gap-2">
            <div
              className={cn(
                ...
                index <= currentStep ? "bg-gift-ink" : "bg-gift-line",
              )}
            />
            <span
              className={cn(
                "hidden truncate text-xs font-bold sm:block",
                index === currentStep ? "text-gift-ink" : "text-gift-muted/60",
              )}
            >
              {index + 1}. {step}
            </span>
